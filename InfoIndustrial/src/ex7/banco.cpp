...
Banco::Banco(int t) //O construtor criara 4 contas
{
    this->tam = t;
    this->top = -1;
    this->contas = new Conta[this->tam];
    this->contas[++top] = Conta(1234, 1, "Joao", "Corrente", 300);
    this->contas[++top] = {4567, 2, "Jose", "Poupanca", 800};
    this->contas[++top] = {7890, 3, "Maria", "Corrente", 1000};
    this->senhaFunc = 4321;
}
...
void Banco::cadastroClientes()
{
    ...
    if(this->senhaFunc == senha){
        ...
        this->contas[++top] = {senhaConta, numeroConta, titular, tipo, valor};
        cout << "Senha criada!" << endl;
        
    }
}

void Banco::removeClientes(Conta& var){
    var = contas[top--];
    //cout << "Cliente da conta " << var << "foi removido!"<<endl;
}
